...
static void request_callback(HINTERNET session, DWORD_PTR ctx, DWORD status, void* info, DWORD ninfo) {
    ...
    case WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE:
      ...
      if(ninfo > req->output_capacity - req->noutput) {
        ...
        if(cap < ninfo) cap = 1024*1024;
        if(cap < ninfo) cap = ninfo;
...
static char* request_reason(Request *req) {
  if(req->error) return "request_run failed";
  if(!req->reason) {
    wchar_t reason[1024];
...
static int request_run(Request *req) {
  RequestUrl u;
  if(request_url_parse(&u, req->url, 0)) return -1;
  int port;
  if(u.nport) port = 0;
  else if(!strncmp(u.scheme, "http", u.nscheme)) port = 80;
  else if(!strncmp(u.scheme, "https", u.nscheme)) port = 443;
...
static void request_destroy(Request *req) {
  ...
  free(req->verb);
  free(req->reason);
  if(req->handle) WinHttpCloseHandle(req->handle);
  free(req->input);
  free(req->output);
